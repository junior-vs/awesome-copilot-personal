...
# Browse marketplace with JSON output (v1.0.84+)
...
**CLI enable/disable *(v1.0.84+)***: You can also toggle plugins, MCP servers, and skills directly from the command line without opening the interactive dashboard:
...
> **Note**: Prior to v1.0.84, enabling and disabling hooks and LSP servers individually was unavailable. Starting with v1.0.84, you can list LSP servers with `copilot lsp list` and instructions with `copilot instruction list`.

> **Dashboard available to everyone (v1.0.81+)**: The plugins dashboard (`/plugin`, `/mcp`, and `/skills`) is now on for all users by default. `/skills`, bare `/mcp`, and `/mcp show` (with no server name) always open the dashboard now, and `/mcp config` opens the dedicated MCP wizard.

### Listing Instructions and LSP Servers

*(v1.0.84+)* Two new dedicated list commands let you inspect loaded instructions and LSP servers from the terminal:
...
These replace the previous `copilot plugins list --kind instruction` and `copilot plugins list --kind lsp` flags, which have been removed.

> **Note (v1.0.84+)**: `copilot plugins list` is now an alias for `copilot plugin list` and reports **only plugins**. It no longer lists MCP servers, skills, instructions, or LSP servers. Use the dedicated commands (`copilot mcp list`, `copilot skill list`, `copilot instruction list`, `copilot lsp list`) for those resource types.
