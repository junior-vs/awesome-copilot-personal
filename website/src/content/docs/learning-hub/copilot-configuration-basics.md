...
| `editorMode` | Composer input mode — set to `"vim"` for modal vim-style editing (v1.0.85+) |
| `transcriptView` | Set to `"concise"` to group tool activity into expandable work summaries in the session transcript (v1.0.85+) |
...
The `/vim` command *(v1.0.85+)* enables vim-style modal editing in the composer. Toggle it on with `/vim` or set `editorMode` to `"vim"` in your config to make it the default. The current mode (NORMAL / INSERT) is shown in the footer while you type:

```
/vim        # toggle vim mode on or off
```

You can also set it persistently in your config:
...
The `/config` command *(v1.0.85+)* opens a sidebar configuration screen directly from within a CLI session — no need to leave the session or edit a file manually. From this sidebar you can inspect and change settings such as the default model, reasoning effort, sandbox rules, and more:

```
/config         # open the configuration sidebar
/config model   # jump to the model section of the configuration sidebar
```
