...
| `editorMode` | Input editing mode for the interactive composer; set to `vim` for modal Vim-style editing (v1.0.84+) |

> **Note**: Older snake_case names (e.g., `include_gitignored`, `auto_updates_channel`) are still accepted for backward compatibility, but camelCase is now the preferred format.

> **Vim mode (v1.0.84+)**: Vim mode is now available to all users. Enable it with the `/vim` slash command inside a session, or by setting `editorMode` to `vim` in your config. The current mode (normal/insert) is shown next to the composer while you type. Use `/vim` to toggle it off again.
