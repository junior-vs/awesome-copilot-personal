...
| `editorMode` | Input editor mode for the composer; set to `vim` for modal editing (v1.0.84+) |
...
> **Vim mode (v1.0.84+)**: Run `/vim` inside a session to toggle vim-style modal editing in the composer, with the current mode (`NORMAL`, `INSERT`) shown while you type. You can also enable it permanently by setting `editorMode` to `vim` in your config. This gives you full keyboard navigation — `hjkl` movement, `i`/`a`/`o` to enter insert mode, `Esc` to return to normal mode — for composing longer prompts without leaving the keyboard.
...
**Session and memory import (v1.0.84+)**: Copilot CLI supports a **semantic JSONL interchange format** for importing sessions and memory across environments. Use the new import commands to move conversation history or memory entries between machines, share them with teammates, or inject them into new sessions:

```bash
copilot session import <file.jsonl>   # import session history from JSONL
copilot memory import <file.jsonl>    # import memory entries from JSONL
```

The JSONL format is structured for portability — each line is a self-contained JSON object representing a turn, tool call, or memory entry. This enables programmatic session transfer, backup workflows, and integration with external tooling that produces compatible output.
...
**Listing loaded resources from the shell (v1.0.84+)**: Use these dedicated subcommands to list loaded resources from outside a session:

```bash
copilot instruction list   # list all active instruction files
copilot lsp list           # list all configured LSP servers
copilot plugin list        # list installed plugins
copilot mcp list           # list configured MCP servers
copilot skill list         # list loaded skills
```

These complement `copilot plugin list --json`, `copilot plugin marketplace list --json`, and `copilot plugin marketplace browse --json` (also v1.0.84+) for scripting and CI integrations that need structured output.

**Enabling and disabling from the shell (v1.0.84+)**: You can enable or disable plugins, MCP servers, and skills directly from the command line without entering an interactive session:

```bash
copilot plugin enable my-plugin     # enable an installed plugin
copilot plugin disable my-plugin    # disable without uninstalling

copilot mcp enable postgres         # enable an MCP server
copilot mcp disable postgres        # disable without removing config

copilot skill enable my-skill       # enable a skill
copilot skill disable my-skill      # disable a skill
```

These replace the older `copilot plugins enable/disable --plugin|--mcp|--skill` flags, which have been removed.
