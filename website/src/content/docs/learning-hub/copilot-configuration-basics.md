...
| `editorMode` | Set to `vim` to enable Vim (modal) editing in the composer (v1.0.84+) |
...
**Vim mode** *(v1.0.84+)*: GitHub Copilot CLI now supports **Vim-style modal editing** in the composer. Enable it with `/vim` in any session, or set `editorMode` to `vim` in your `config.json` for it to apply by default. When active, the current editing mode (insert or normal) is shown while you type, so you can navigate and edit prompts without reaching for the mouse. To turn it off, run `/vim` again or set `editorMode` back to its default.
...
```
 ...
 opens a timeline picker that lets you roll back the conversation to any earlier point in history. You can also trigger it by pressing **double-Esc**:
