...
*(v1.0.85+)*: You can also opt in to **context management tools** for agents and subagents via `/settings`. When enabled, this lets the agent actively manage its own context window — trimming, summarising, or restructuring conversation history — to stay within token limits during long multi-step tasks without losing critical information.
