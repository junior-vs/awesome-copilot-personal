...
${CONCISE_MODE=true|false}
<!-- If true, generate a concise summary suitable for small/simple projects -->

${ON_DETECTION_FAILURE="fallback|error|prompt"}
<!-- What to do if project type or structure cannot be detected: fallback to generic, show error, or prompt user for clarification -->
...
If the project type or structure cannot be detected:
- If ${ON_DETECTION_FAILURE} is "fallback", generate a generic folder structure analysis using best practices for unknown tech stacks.
- If ${ON_DETECTION_FAILURE} is "error", output a clear error message indicating what information is missing or ambiguous, and suggest how the user can provide more context.
- If ${ON_DETECTION_FAILURE} is "prompt", ask the user for clarification about the project type or structure before proceeding.

If ${CONCISE_MODE} is true, generate a brief summary focusing only on the most important folders, entry points, and conventions, omitting deep analysis and advanced sections.
...
- Review and update this document whenever the project structure changes significantly (e.g., new tech stack, major refactor, monorepo split/merge).
- Record the date and author of each update in a changelog section at the end of the blueprint.
- Encourage team members to suggest improvements or corrections as the codebase evolves.
- For automated projects, consider integrating structure validation tools that can flag drift from the documented blueprint.

### Versioning
- Use semantic versioning for the blueprint document (e.g., v1.0.0, v1.1.0) and increment the version with each major or minor change.
- Optionally, keep previous versions of the blueprint in a /docs/history or /blueprints/ folder for reference.

### Review and Accuracy
- After generating or updating the blueprint, review the output for accuracy, relevance, and conciseness—especially if using concise mode or fallback handling.
- For ambiguous or custom structures, supplement the generated output with manual notes or clarifications.
